...
class PlaylistItem:
    def __init__(self, 
        ...
        self.outPath = rename_filename_keep_extension(outPath, title)  # Path to music dir where the music should be copied to.
...
def get_playlists(plex: PlexServer):
    ...
    print('got %d playlists' % len(result))
    return result
...
def copy_files(playlistItems: list[PlaylistItem]):
    for i, value in enumerate(playlistItems):
        print("[%i/%i] %s..." % (i+1, len(playlistItems), value.title), end='', flush=True)
        try:
            if copy_file_if_newer(value.fsPath, value.outPath): 
                print(' Copied!')
            else:
                print(' Up to date!')  
...
def main():
    ...
    print('')
    print('Created all playlists, going to copy music.')
    copy_files(all_songs)
    
    print('')
    print("Checking album art")
    parse_album_art(music_dir)
